    ...
    try {
      ...
      if (response.ok) {
        setDashboardData({
          ...
          overallScore: Math.min(data.overallScore, 100),
          previousScore: Math.min(data.previousScore, 100),
          assessmentsCompleted: data.totalAssessments,
          totalAssessments: 10,
          lastAssessment: data.lastAssessment,
          categories: Object.entries(data.categoryScores || {}).map(([name, score]) => ({
            name,
            score: Math.min(Number(score), 100),
            status: score >= 80 ? 'good' : 'warning'
          })),
          achievements: [
            { name: 'Security Rookie', icon: Shield, earned: data.overallScore >= 60 },
            { name: 'Password Master', icon: CheckCircle, earned: (data.categoryScores['Password Management'] || 0) >= 80 },
            { name: 'Privacy Pro', icon: Award, earned: data.overallScore >= 90 },
          ],
          recentActivity: data.attempts.map(attempt => ({
            action: `Completed Security Assessment (Attempt #${attempt.attemptNumber})`,
            date: new Date(attempt.completedAt).toLocaleDateString(),
            score: Math.min(attempt.overallPercentage, 100),
