...
router.post('/submit', async (req, res) => {
  ...
  try {
    ...
    const attemptCategories = user.categoryScores.filter(cs => cs.attemptNumber === currentAttempt && cs.questionsAnswered > 0);
...
router.get('/questions', async (req, res) => {
  try {
    ...
    // Fetch 10 questions, ensuring at least one from each category if possible
    const allCategories = [
      'Password Management',
      'Authentication',
      'Device Security',
    ...
    // If fewer than 10 questions, fill with random questions
    if (questions.length < 10) {
      const additionalQuestions = await Question.find({
        userType: { $in: [userType, 'both'] },
        isActive: true,
        _id: { $nin: [...answeredQuestionIds.map(id => new mongoose.Types.ObjectId(id)), ...questions.map(q => q._id)] }
