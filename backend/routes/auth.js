...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    await auth.save();
    console.log(`Created Auth document with _id: ${auth._id}`);
    ...
    if (!user) {
      ...
      await user.save();
      console.log(`Created User document with userId: ${auth._id.toString()}`);
    } else {
      console.log(`User document already exists for userId: ${auth._id.toString()}`);
    }
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      console.log(`User document found for userId: ${auth._id.toString()}`);
    }

    const token = jwt.sign(
      { userId: auth._id.toString(), userType: auth.userType, name: auth.profile.name, email: auth.email },
